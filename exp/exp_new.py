...
def compute_entropy_loss(current_ratio, eps=1e-8):
    entropy = -(current_ratio * torch.log(current_ratio + eps)).sum()
    max_entropy = math.log(len(current_ratio))
    return 1.0 - (entropy / max_entropy)


def compute_region_diversity_loss(cls_soft, eps=1e-8):
    region_entropies = -(cls_soft * torch.log(cls_soft + eps)).sum(dim=-1)
    max_entropy = math.log(cls_soft.size(-1))
    normalized_entropies = region_entropies / max_entropy
    return 1.0 - normalized_entropies.mean()
...
class Exp_TimeMosaic(Exp_Basic):
    ...
    def train(self, setting):
        ...
        lam_entropy = getattr(self.args, 'lam_entropy', 0.03)
        lam_diversity = getattr(self.args, 'lam_diversity', 0.008)
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        lambda_cls = 0.01
                        loss = criterion(outputs, batch_y) + lambda_cls * criterion(current_ratio, torch.full_like(current_ratio, 1.0 / len(current_ratio)).detach())

                        if self.args.mask_ratio > 0:
                            loss = loss + criterion(batch_x, dec_mask)

                        cls_soft_all = self.model.patch_embedding.latest_cls_soft
                        if cls_soft_all is not None:
                            avg_ratio = cls_soft_all.mean(dim=0)
                            loss = loss + lam_entropy * compute_entropy_loss(avg_ratio)
                            loss = loss + lam_diversity * compute_region_diversity_loss(cls_soft_all)
