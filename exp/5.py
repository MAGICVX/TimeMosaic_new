    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                # Channel shuffling for permutation invariance (CPiRi-inspired)
                if self.args.channel_shuffle:
                    perm = torch.randperm(batch_x.size(2), device=batch_x.device)
                    batch_x = batch_x.index_select(2, perm)
                    batch_y = batch_y.index_select(2, perm)
