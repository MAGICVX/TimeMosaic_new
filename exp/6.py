...
class Exp_TimeMosaic(Exp_Basic):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        outputs, cls_pred = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[:2]
                else:
                    outputs, cls_pred = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[:2]
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        result = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, mask, batch_y)
                        outputs, cls_pred, dec_mask = result
                        ...
                        f_dim = -1 if self.args.features == 'MS' else 0
                        outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        batch_y_trim = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                        lambda_cls = 0.01
                        loss = criterion(outputs, batch_y_trim) + lambda_cls * criterion(
                            current_ratio, torch.full_like(current_ratio, 1.0 / len(current_ratio)).detach())
                        if self.args.mask_ratio > 0:
    ...
    def profile_model(self, test_loader):
        ...
        with torch.no_grad():
            ...
            _ = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[:2]
