...
class Model(nn.Module):
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # === MoE dynamic prompt (replaces static nn.Embedding) ===
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        # === Generate input-conditioned prompts via MoE ===
        prompts, moe_routing_weights = self.prompt_moe(enc_out, extra_token)
        # prompts: [num_segs, B*C, num_latent_token, d_model]

        for i in range(self.num_segs):
            prompt = prompts[i]  # [B*C, num_latent_token, d_model]

            # Concatenate prompt, patch embeddings, global channel
            segment_input = torch.cat([prompt, enc_out], dim=1)  # [B*C, prompt+patch+extra, d_model]
        ...
        return dec_out, cls_pred, dec_mask, moe_routing_weights

    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        dec_out, cls_pred, dec_mask, moe_routing_weights = self.forecast(
            x_enc, x_mark_enc, x_dec, x_mark_dec, mask)
        if self.training > 0:
            return dec_out[:, -self.pred_len:, :], cls_pred, dec_mask, moe_routing_weights
        else:
