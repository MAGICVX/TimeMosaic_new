...
class AdaptivePatchEmbedding(nn.Module):
    def __init__(self, d_model, patch_len_list, mode='fixed', dropout=0.0, seq_len=96, in_channels=1, training=True):
        ...
        self.freq_cls = nn.Sequential(
            nn.Linear(4, 16),
            nn.ReLU(),
            nn.Linear(16, len(patch_len_list))
        )
    ...
    def forward(self, x):  # x: [B, C, L]
        ...
        for region_idx in range(self.region_num):
            ...
            # Frequency features → additive logits
            region_freq = torch.fft.rfft(region, dim=-1).abs()
            freq_abs = region_freq + 1e-8
            dom_freq = freq_abs.argmax(dim=-1).float() / freq_abs.shape[-1]
            freq_norm = freq_abs / freq_abs.sum(dim=-1, keepdim=True)
            spec_ent = -(freq_norm * freq_norm.log()).sum(dim=-1)
            low_ratio = freq_abs[:, :4].sum(dim=-1) / freq_abs.sum(dim=-1)
            total_en = freq_abs.sum(dim=-1).log()
            freq_feat = torch.stack([dom_freq, spec_ent, low_ratio, total_en], dim=-1)
            cls_logits = cls_logits + self.freq_cls(freq_feat)
...
class DualFlattenHead(nn.Module):
    """Dual-path head: trend (pooled) + detail (full resolution)."""
    def __init__(self, n_vars, nf, target_window, d_model, head_dropout=0):
        super().__init__()
        self.n_vars = n_vars
        self.trend_linear = nn.Linear(d_model, target_window)
        self.flatten = nn.Flatten(start_dim=-2)
        self.detail_linear = nn.Linear(nf, target_window)
        self.dropout = nn.Dropout(head_dropout)

    def forward(self, x):  # x: [B, nvars, d_model, patch_num]
        trend = x.mean(dim=-1)
        trend = self.trend_linear(trend)
        detail = self.flatten(x)
        detail = self.detail_linear(detail)
        detail = self.dropout(detail)
        return trend + detail
...
class Model(nn.Module):
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        # Channel Mixer: lightweight cross-channel interaction after temporal encoding
        self.channel_mixer = nn.Linear(configs.enc_in, configs.enc_in, bias=False)
        nn.init.eye_(self.channel_mixer.weight)
        ...
        self.prompt_token = nn.Embedding(self.num_latent_token * self.num_segs, self.d_model)
        nn.init.xavier_uniform_(self.prompt_token.weight)
        ...
        self.heads = nn.ModuleList([
            DualFlattenHead(configs.enc_in, self.head_nf, self.seg_len, configs.d_model, head_dropout=configs.dropout)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask, batch_y=None):
        ...
        for i in range(self.num_segs):
            # Static prompt for segment i
            prompt = self.prompt_token.weight[
                i * self.num_latent_token : (i + 1) * self.num_latent_token
            ].unsqueeze(0).repeat(B * C, 1, 1)  # [B*C, num_prompt, d_model]
            ...
            # Channel Mixer: cross-channel interaction after temporal encoding
            # [B*C, patch_num, d] → [B, patch_num*d, C] → Linear(C,C) → [B, patch_num*d, C] → [B, C, d, patch_num]
            segment_out = segment_out.reshape(B, C, self.patch_num * self.d_model)
            segment_out = self.channel_mixer(segment_out.transpose(1, 2)).transpose(1, 2)
            segment_out = segment_out.reshape(B, C, self.d_model, self.patch_num)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, batch_y=None):
        dec_out, cls_pred, dec_mask = self.forecast(
            x_enc, x_mark_enc, x_dec, x_mark_dec, mask, batch_y)
        if self.training > 0:
