...
class TemporalPromptEnhancer(nn.Module):
    ...
    """
    def __init__(self, d_model, num_layers=2, num_heads=4, dim_feedforward=2048, dropout=0.1):
        super().__init__()
...
class DynamicPromptGenerator(nn.Module):
    ...
    优化版本：加入时序增强模块 (TemporalPromptEnhancer)
    先对 patch 序列进行时序建模，再基于增强后的特征生成 prompt
    """
    def __init__(self, d_model, num_latent_token, num_segs, hidden_dim=512, num_temporal_layers=4, num_temporal_heads=8):
        super().__init__()
        self.d_model = d_model
        self.num_latent_token = num_latent_token
        ...
        self.feature_extractor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, num_segs * num_latent_token * d_model)
        )
    ...
    def forward(self, x):
        ...
        # 对增强后的特征进行池化，获取全局特征
        global_feature = enhanced_x.mean(dim=1)  # [B*C, d_model]
        
        # 生成所有段的 prompt
        prompt_params = self.feature_extractor(global_feature)  # [B*C, num_segs*num_latent_token*d_model]
