# 强化特征提取器（调参版） dim_feedforward=d_model*4 --> *2
...
class TemporalPromptEnhancer(nn.Module):
    """时序增强模块：使用轻量级 Transformer 编码器增强 patch 序列的时序特征
    
    作用：在生成动态 prompt 之前，先对 patch 序列进行时序建模
    捕获 patch 之间的依赖关系，使生成的 prompt 更有判别性
    """
    def __init__(self, d_model, num_layers=2, num_heads=4, dim_feedforward=512, dropout=0.1):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            activation='gelu',
            batch_first=True,
            norm_first=True  # Pre-LN 格式，更利于训练稳定
        )
    ...
    def forward(self, x):
        """
        x: [B*C, patch_num, d_model] - patch 嵌入序列
        return: [B*C, patch_num, d_model] - 增强后的时序特征
        """
        return self.temporal_encoder(x)


class DynamicPromptGenerator(nn.Module):
    """动态 Prompt 生成器：根据输入序列特征生成 adaptive prompts
    
    优化版本：
    1. 时序增强模块 (TemporalPromptEnhancer) - 捕获 patch 之间的时序依赖
    2. 强化特征提取器 (EnhancedFeatureExtractor) - 更强的特征变换能力
    """
    def __init__(self, d_model, num_latent_token, num_segs, hidden_dim=512, num_temporal_layers=16, num_temporal_heads=8):
        super().__init__()
        self.d_model = d_model
        self.num_latent_token = num_latent_token
        ...
        # 时序增强模块
        self.temporal_enhancer = TemporalPromptEnhancer(
            d_model=d_model,
    ...
    def forward(self, x):
        """
        x: [B*C, patch_num, d_model] - 补丁嵌入
        return: [num_segs, B*C, num_latent_token, d_model] - 每个段的动态 prompt
        """
        # 时序增强
        enhanced_x = self.temporal_enhancer(x)  # [B*C, patch_num, d_model]
        
        # 生成动态 prompt
        prompt_params = self.feature_extractor(enhanced_x)
        
        return prompt_params


class EnhancedFeatureExtractor(nn.Module):
    """强化特征提取器：使用残差连接和层归一化增强特征变换能力
    ...
    3. GELU 激活函数
    """
    def __init__(self, d_model, num_segs, num_latent_token, hidden_dim=512):
        super().__init__()
        self.d_model = d_model
        self.num_segs = num_segs
        self.num_latent_token = num_latent_token
        ...
        # 输出投影
        self.output_proj = nn.Linear(hidden_dim, num_segs * num_latent_token * d_model)
        
    def forward(self, x):
        """
        x: [B*C, patch_num, d_model]
        return: [num_segs, B*C, num_latent_token, d_model]
        """
        # 全局池化
        global_feature = x.mean(dim=1)  # [B*C, d_model]
        
        # 输入投影
        h = self.input_proj(global_feature)
        ...
        # 输出投影并 reshape
        prompt_params = self.output_proj(h)  # [B*C, num_segs*num_latent_token*d_model]
        prompt_params = prompt_params.view(-1, self.num_segs, self.num_latent_token, self.d_model)  # [B*C, num_segs, num_latent_token, d_model]
        prompt_params = prompt_params.permute(1, 0, 2, 3)  # [num_segs, B*C, num_latent_token, d_model]
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        # 生成动态 prompt
        dynamic_prompts = self.dynamic_prompt_generator(enc_out[:, :self.patch_num, :])  # [num_segs, B*C, num_latent_token, d_model]

        for i in range(self.num_segs):
            # 使用动态生成的 prompt
            prompt = dynamic_prompts[i]  # [B*C, num_latent_token, d_model]
