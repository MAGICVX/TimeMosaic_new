"""
TimeMosaic x SEMPO Fusion V3 Architecture.
Kairos MoS hierarchical patching + DRoPE + learnable-bias prefix.

Compared to V2 (TimeMosaic_FusionV2.py):
  1. MoS (Mixture-of-Size) hierarchical patch tokenization
     -> Replaces AdaptivePatchEmbeddingV2's Gumbel-Softmax soft selection
     -> Hard MoE routing with learnable bias determines patch granularity
     -> Hierarchical splitting: coarse patches split based on content complexity
     -> Per-granularity linear embeddings (32/16/8-size patches)
     -> Multi-view routing consistency: raw view routes, freq views reuse

  2. DRoPE (Instance-Wise Rotary Position Embedding)
     -> Same as V2

  3. Static prompts (original TimeMosaic style)
     -> Same as V2

  4. MoE attention prefix with learnable bias (auto-balancing)
     -> Same as V2

Preserves:
  - Spectral decomposition (SEMPO)
  - MoE attention prefix K/V injection
  - Reconstruction auxiliary task (removed in this version for simplicity)
  - Multi-view cross-aggregation
  - DRoPE in attention layers
"""
...
from layers.MoSPatchEmbedding import MoSPatchEmbedding
...
class RoPEEncoderLayer(nn.Module):
    ...
    Like V2's RoPEEncoderLayer — forwards rope and s_features to attention.
    """
...
class RoPEPrefixEncoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None, prefix_kv=None, rope=None,
        ...
        for i, layer in enumerate(self.attn_layers):
            ...
            x, _ = layer(x, attn_mask=attn_mask, prefix_kv=pkv,
                         rope=rope, s_features=s_features)
...
class Model(nn.Module):
    """
    TimeMosaic x SEMPO Fusion V3.

    Key innovation: MoS (Mixture-of-Size) hierarchical patching
    replaces Gumbel-Softmax soft selection. Each region is routed to a
    granularity level via hard MoE gating with learnable bias, then
    hierarchically split. Per-granularity Linear layers embed patches
    of different sizes.

    Improvements over V2:
      - MoS hard routing (no wasted computation on unselected granularities)
      - Learnable bias load balancing (no auxiliary loss for MoS)
      - Hierarchical splitting adapts patch count to content complexity
    """
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        if self.use_drope:
            ...
            self.rope = InstanceWiseRotaryEmbedding(
                dim=configs.d_model // configs.n_heads,
        ...
        if self.freq_num > 0:
            ...
            self.tau_main = nn.Parameter(
                torch.rand(self.freq_num) * (configs.seq_len // 2 + 1))
            self.mu_main = nn.Parameter(
                torch.bernoulli(torch.full((self.freq_num, self.freq_len), 0.5)))
            self.tau_res = nn.Parameter(
                torch.rand(self.freq_num) * (configs.seq_len // 2 + 1))
            self.mu_res = nn.Parameter(
                torch.bernoulli(torch.full((self.freq_num, self.freq_len), 0.5)))
        self.num_views = self.freq_num + 1

        # ── MoS Patch Tokenizer (replaces AdaptivePatchEmbedding) ──
        self.mos_max_patch_len = getattr(configs, 'mos_max_patch_len', 32)
        self.mos_levels = getattr(configs, 'mos_levels', 3)
        self.mos_bias_rate = getattr(configs, 'mos_update_bias_rate', 0.001)

        self.patch_embedding = MoSPatchEmbedding(
            max_patch_len=self.mos_max_patch_len,
            levels=self.mos_levels,
            d_model=configs.d_model,
            seq_len=configs.seq_len,
            update_bias_rate=self.mos_bias_rate,
            dropout=configs.dropout,
            training=configs.is_training)

        # Compute fixed patch count for FlattenHead
        self.patch_num = self.patch_embedding.max_total_patches
        self.head_nf = configs.d_model * self.patch_num

        # ── Cross-view aggregation ──
        self.view_projection = nn.Linear(
            self.num_views * configs.d_model, configs.d_model)
        ...
        self.prefix_bias_rate = getattr(configs, 'moe_bias_rate', 0.001)
        self.use_prefix = getattr(configs, 'use_prefix', True)
        if self.use_prefix:
            self.prefix_moe = MoEPrefixGeneratorV2(
                d_model=configs.d_model,
                num_experts=getattr(configs, 'num_moe_prefix_experts', 4),
                n_layers=configs.e_layers,
                prefix_len=self.prefix_len,
                hidden_size=configs.d_model,
                dropout=configs.dropout,
                update_bias_rate=self.prefix_bias_rate)
        else:
        ...
        # ── Masking / RevIN ──
        self.revin = getattr(configs, 'revin', False)
        if self.revin:
    ...
    # ─── Spectral Decomposition (same as V1/V2) ─────────────────────
    ...
    # ─── Multi-view MoS patching ───────────────────────────────────

    def _patch_view(self, x_view, reuse_expert_indices=None):
        """
        x_view: [B, T, C] -> patches: [B*C, max_total_patches, D]
        Uses MoSPatchEmbedding with optional routing reuse.
        Returns (patches, cls_pred, expert_indices) — cls_pred for monitoring.
        """
        x_flat = x_view.permute(0, 2, 1).reshape(-1, x_view.shape[1])
        patches, C, cls_pred, expert_indices = self.patch_embedding(
            x_flat, reuse_expert_indices=reuse_expert_indices)
        return patches, C, cls_pred, expert_indices
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        # ── DRoPE: extract spectral features ──
        if self.use_drope and self.rope is not None:
            s_features = self.fft_extractor(x_enc)
        ...
        # ── Multi-view MoS patching ──
        # Raw view routes; freq views reuse the same routing for consistency
        all_patches = []
        cls_preds = []

        patches, _, cp, routing = self._patch_view(x_enc, reuse_expert_indices=None)
        all_patches.append(patches)
        cls_preds.append(cp)

        for i in range(self.freq_num):
            patches, _, cp, _ = self._patch_view(
                x_freq[i], reuse_expert_indices=routing)
            all_patches.append(patches)
            cls_preds.append(cp)
        ...
        seg_outputs = []
        total_len = enc_out.shape[1]  # P + extra_len
        N = self.num_latent_token
        for i in range(self.num_segs):
            prompt = prompts[i]
            segment_input = torch.cat([prompt, enc_out], dim=1)
            segment_out = self.encoder(
                segment_input, prefix_kv=prefix_kv,
                rope=self.rope, s_features=s_features)
            segment_out = segment_out[:, N:N + self.patch_num, :]
            segment_out = torch.reshape(
                segment_out, (B, C, self.d_model, self.patch_num))
